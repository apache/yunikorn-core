...
func TestTryAllocatePreemptNode(t *testing.T) {
	...
	alloc1 := app1.tryAllocate(resources.NewResourceFromMap(map[string]resources.Quantity{"first": 18}), 30*time.Second, &preemptionAttemptsRemaining, iterator, iterator, getNode)
	assert.Assert(t, alloc1 != nil, "alloc1 expected")
	alloc2 := app1.tryAllocate(resources.NewResourceFromMap(map[string]resources.Quantity{"first": 13}), 30*time.Second, &preemptionAttemptsRemaining, iterator, iterator, getNode)
	assert.Assert(t, alloc2 != nil, "alloc2 expected")
	...
	alloc3 := app2.tryAllocate(resources.NewResourceFromMap(map[string]resources.Quantity{"first": 8}), 30*time.Second, &preemptionAttemptsRemaining, iterator, iterator, getNode)
