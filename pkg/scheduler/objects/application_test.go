...
func TestAppReservation(t *testing.T) {
	...
	askKey := "alloc-1"
	ask := objects.newAllocationAsk(askKey, appID, res)
	nodeID := "node-1"
...
func TestAppAllocReservation(t *testing.T) {
	...
	nodeID1 := "node-1"
	res := resources.NewResourceFromMap(map[string]resources.Quantity{"first": 5})
	ask := objects.newAllocationAskRepeat(allocKey, appID, res, 2)
...
func TestUpdateRepeat(t *testing.T) {
	...
	allocKey := "alloc-1"
	res := resources.NewResourceFromMap(map[string]resources.Quantity{"first": 5})
	ask := objects.newAllocationAskRepeat(allocKey, appID, res, 1)
	delta, err = app.addAllocationAsk(ask)
	if err != nil || !resources.Equals(res, delta) {
		t.Errorf("ask should have been added to app, err %v, expected delta %v but was: %v", err, res, delta)
	}
...
func TestAddAllocAsk(t *testing.T) {
	...
	ask := objects.newAllocationAsk(allocKey, appID, res)
	delta, err = app.addAllocationAsk(ask)
	if err == nil {
		t.Errorf("zero resource ask should not have been added to app, returned delta: %v", delta)
	}
	res = resources.NewResourceFromMap(map[string]resources.Quantity{"first": 5})
	ask = objects.newAllocationAskRepeat(allocKey, appID, res, 0)
	delta, err = app.addAllocationAsk(ask)
	if err == nil {
	...
	ask = objects.newAllocationAskRepeat(allocKey, appID, resources.NewResourceFromMap(map[string]resources.Quantity{"first": 3}), 5)
	delta, err = app.addAllocationAsk(ask)
	if err != nil || !resources.Equals(res, delta) {
		t.Errorf("ask should have been added to app, err %v, expected delta %v but was: %v", err, res, delta)
	}
...
func TestRemoveAllocAsk(t *testing.T) {
	...
	ask = objects.newAllocationAskRepeat("alloc-2", appID, res, 2)
...
func TestSortRequests(t *testing.T) {
	...
	for i := 1; i < 4; i++ {
		num := strconv.Itoa(i)
		ask := objects.newAllocationAsk("ask-"+num, appID, res)
...
func TestStateChangeOnAskUpdate(t *testing.T) {
	...
	res := resources.NewResourceFromMap(map[string]resources.Quantity{"first": 1})
	askID := "ask-1"
	ask := objects.newAllocationAsk(askID, appID, res)
	...
	ask = objects.newAllocationAskRepeat(askID, appID, res, 0)
