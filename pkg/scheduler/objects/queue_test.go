...
func TestQuotaPreemptionSettings(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NilError(t, err, "failed to apply conf: %v", err)
			assert.Assert(t, resources.Equals(oldMax, expectedMax), "old max not from setup")
...
func TestNewConfiguredQueue(t *testing.T) {
	...
	assert.Equal(t, parent.quotaPreemptionDelay, configs.DefaultQuotaPreemptionDelay)
...
func TestQueue_setPreemptionTime(t *testing.T) {
	...
	tests := []struct {
		...
		{"max lowered but usage (including reservations) fits with in newer max", resources.NewResourceFromMap(map[string]resources.Quantity{"first": 100}), map[string]string{"first": "20"}, resources.NewResourceFromMap(map[string]resources.Quantity{"first": 5}), 10, 10, true, true, 1},
		...
		{"max increase but usage (including reservations) fits with in newer max", resources.NewResourceFromMap(map[string]resources.Quantity{"first": 10}), map[string]string{"first": "100"}, resources.NewResourceFromMap(map[string]resources.Quantity{"first": 90}), 5, 5, true, true, 1},
		{"max increased 2nd with delay change but usage (including reservations) fits with in newer max", resources.NewResourceFromMap(map[string]resources.Quantity{"first": 100}), map[string]string{"first": "120"}, resources.NewResourceFromMap(map[string]resources.Quantity{"first": 110}), 5, 10, true, true, 1},
		...
		{"max lowered again but usage (including reservations) fits with in newer max", resources.NewResourceFromMap(map[string]resources.Quantity{"first": 100}), map[string]string{"first": "20"}, resources.NewResourceFromMap(map[string]resources.Quantity{"first": 5}), 10, 10, true, true, 1},
