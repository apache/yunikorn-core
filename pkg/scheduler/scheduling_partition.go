...
type PartitionSchedulingContext struct {
    ...
    nodes            map[string]*SchedulingNode        // nodes belonged to this partition
...
// Add scheduling node to the scheduling partition.
func (psc *PartitionSchedulingContext) AddSchedulingNode(schedulingNode *SchedulingNode) error {
    psc.lock.Lock()
    defer psc.lock.Unlock()

    // Add to nodes
    nodeId := schedulingNode.NodeId
    if psc.nodes[nodeId] != nil {
        return fmt.Errorf("adding node %s to partition %s, but node already existed", nodeId, psc.Name)
    }
    // update partition
    psc.nodes[nodeId] = schedulingNode

    return nil
}

// Remove scheduling node from the scheduling partition.
func (psc *PartitionSchedulingContext) RemoveSchedulingNode(nodeId string) (*SchedulingNode, error) {
    psc.lock.Lock()
    defer psc.lock.Unlock()

    // Remove from nodes map
    if psc.nodes[nodeId] == nil {
        return nil, fmt.Errorf("removing node %s from partition %s, but nodes does not exist", nodeId, psc.Name)
    }
    schedulingNode := psc.nodes[nodeId]
    delete(psc.nodes, nodeId)

    return schedulingNode, nil
}

// Get scheduling node from the scheduling partition.
