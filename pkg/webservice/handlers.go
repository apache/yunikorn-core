...
func getPartitionQueue(w http.ResponseWriter, r *http.Request) {
	...
	queue := partitionContext.GetQueue(escapedQueueName)
	if queue == nil {
...
func getUserResourceUsage(w http.ResponseWriter, r *http.Request) {
	...
	escapedUser, err := url.QueryUnescape(user)
	if err != nil {
		buildJSONErrorResponse(w, err.Error(), http.StatusBadRequest)
		return
	}
	userTracker := ugm.GetUserManager().GetUserTracker(escapedUser)
	if userTracker == nil {
...
func getGroupResourceUsage(w http.ResponseWriter, r *http.Request) {
	...
	escapedGroupName, err := url.QueryUnescape(group)
	if err != nil {
		buildJSONErrorResponse(w, err.Error(), http.StatusBadRequest)
		return
	}
	groupTracker := ugm.GetUserManager().GetGroupTracker(escapedGroupName)
	if groupTracker == nil {
