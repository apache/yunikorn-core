...
func newTestPartition() (*PartitionContext, error) {
	rootSched, err := objects.createRootQueue(nil)
	if err != nil {
		return nil, err
	}
...
func TestGetQueue(t *testing.T) {
	...
	_, err = objects.createManagedQueue(queue, "parent", true, resMap)
...
func createQueuesNodes(t *testing.T) *PartitionContext {
	...
	// create the root
	var root, parent *objects.Queue
	resMap := map[string]string{"first": "100"}
	root, err = objects.createRootQueue(resMap)
	assert.NilError(t, err, "failed to create basic root queue")
	// fake adding the queue structure, just add the root
	partition.root = root
	parent, err = objects.createManagedQueue(root, "parent", true, nil)
	assert.NilError(t, err, "failed to create parent queue")
	_, err = objects.createManagedQueue(parent, "leaf1", false, nil)
	assert.NilError(t, err, "failed to create leaf queue")
	_, err = objects.createManagedQueue(root, "leaf2", false, nil)
	assert.NilError(t, err, "failed to create parent queue")
	return partition
}
...
func TestTryAllocate(t *testing.T) {
	...
	delta, err = app.addAllocationAsk(objects.newAllocationAsk("alloc-1", appID1, res))
	if err != nil || !resources.Equals(res, delta) {
		t.Errorf("failed to add ask to app resource added: %v expected %v (err = %v)", delta, res, err)
	}
	ask1 := objects.newAllocationAsk("alloc-2", appID1, res)
	if ask1 == nil {
	...
	ask2 := objects.newAllocationAsk("alloc-1", appID2, res)
...
func TestTryAllocateLarge(t *testing.T) {
	...
	var delta *resources.Resource
	delta, err = app.addAllocationAsk(objects.newAllocationAsk("alloc-1", appID, res))
	if err != nil || !resources.Equals(res, delta) {
		t.Errorf("failed to add ask to app resource added: %v expected %v (err = %v)", delta, res, err)
	}
	alloc := partition.tryAllocate()
...
func TestAllocReserveNewNode(t *testing.T) {
	...
	partition.applications[appID] = app
	var delta *resources.Resource
	ask := objects.newAllocationAskRepeat("alloc-1", appID, res, 2)
	...
	assert.Equal(t, objects.reserved, alloc.result, "allocation result should have been reserved")
...
func TestScheduleRemoveReservedAsk(t *testing.T) {
	...
	partition.applications[appID] = app
	var delta *resources.Resource
	ask := objects.newAllocationAskRepeat("alloc-1", appID, res, 4)
	delta, err = app.addAllocationAsk(ask)
	...
	ask = objects.newAllocationAskRepeat("alloc-2", appID, res, 1)
	delta, err = app.addAllocationAsk(ask)
	if err != nil || !resources.Equals(res, delta) {
		t.Errorf("failed to add ask 2 to app resource added: %v expected %v (err = %v)", delta, resources.Multiply(res, 2), err)
	}
	ask = objects.newAllocationAskRepeat("alloc-3", appID, res, 1)
	delta, err = app.addAllocationAsk(ask)
	if err != nil || !resources.Equals(res, delta) {
	...
	for i := 1; i <= 2; i++ {
		alloc := partition.tryAllocate()
		if alloc == nil || alloc.result != objects.reserved {
