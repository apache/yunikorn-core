...
func (csc *ClusterSchedulingContext) AddSchedulingNode(schedulingNode *SchedulingNode) error {
    partitionName := schedulingNode.NodeInfo.Partition
    nodeId := schedulingNode.NodeId

    csc.lock.Lock()
    defer csc.lock.Unlock()

    if partition := csc.partitions[partitionName]; partition != nil {
        if err := partition.AddSchedulingNode(schedulingNode); err != nil {
            return err
        }
    } else {
        return fmt.Errorf("failed to find partition=%s while adding node=%s", partitionName, nodeId)
    }

    return nil
}

func (csc *ClusterSchedulingContext) RemoveSchedulingNode(nodeId string, partitionName string) (*SchedulingNode, error) {
    csc.lock.Lock()
    defer csc.lock.Unlock()

    if partition := csc.partitions[partitionName]; partition != nil {
        schedulingNode, err := partition.RemoveSchedulingNode(nodeId)
        if err != nil {
            return nil, err
        }
        return schedulingNode, nil
    } else {
        return nil, fmt.Errorf("failed to find partition=%s while remove node=%s", partitionName, nodeId)
    }
}

func (csc *ClusterSchedulingContext) GetSchedulingNode(nodeId string, partitionName string) (*SchedulingNode, error) {
    csc.lock.Lock()
    defer csc.lock.Unlock()

    if partition := csc.partitions[partitionName]; partition != nil {
        return partition.GetSchedulingNode(nodeId), nil
    } else {
        return nil, fmt.Errorf("failed to find partition=%s while remove node=%s", partitionName, nodeId)
    }
}