...
type Queue struct {
	...
	// set before the queue is added to its parent and never changed, no lock needed
	// +checklocksunguarded
...
func NewConfiguredQueue(conf configs.QueueConfig, parent *Queue, silence bool, appQueueMapping *AppQueueMapping) (*Queue, error) {
	...
	// set the events before the queue is added to the parent as that publishes the queue
	if !silence {
	...
	if !silence {
		...
		// YUNIKORN-3419: isManaged read after the queue is published to its parent
...
func convertDelay(value string, def time.Duration) (time.Duration, error) {
	...
	if int64(result) <= int64(0) {
		return def, fmt.Errorf("delay must be positive value: %s", value)
	}
	return result, nil
}
...
// +checklocks:sq.RWMutex
func (sq *Queue) applyConf(conf configs.QueueConfig, silence bool) (*resources.Resource, error) {
	// Set the ACLs
	var err error
...
func (sq *Queue) setPreemptionTime(oldMaxResource *resources.Resource, oldDelay time.Duration) {
	...
	if resources.Equals(oldMaxResource, sq.maxResource) {
		if sq.quotaPreemptionStartTime.IsZero() {
			// if a delay is set later than the quota change we would like to trigger preemption even if no change for
			// max resources is detected. preemption trigger will clean up if usage is below max already.
			if oldDelay == 0 && sq.quotaPreemptionDelay > 0 {
				sq.quotaPreemptionStartTime = time.Now().Add(sq.quotaPreemptionDelay)
				log.Log(log.SchedQueue).Info("set quota preemption start time based on delay change",
					zap.String("queue", sq.QueuePath),
					zap.Time("quotaPreemptionStartTime", sq.quotaPreemptionStartTime))
			}
		} else {
			if oldDelay != sq.quotaPreemptionDelay {
				// apply the delta: this can be positive or negative does not matter. The resulting time could be in the
				// past: also not an issue.
				sq.quotaPreemptionStartTime = sq.quotaPreemptionStartTime.Add(sq.quotaPreemptionDelay - oldDelay)
				log.Log(log.SchedQueue).Info("updated quota preemption start time based on delay change",
					zap.String("queue", sq.QueuePath),
					zap.Time("quotaPreemptionStartTime", sq.quotaPreemptionStartTime),
	...
	if resources.StrictlyGreaterThan(sq.maxResource, oldMaxResource) {
		if !sq.quotaPreemptionStartTime.IsZero() {
			if oldDelay != sq.quotaPreemptionDelay {
				sq.quotaPreemptionStartTime = sq.quotaPreemptionStartTime.Add(sq.quotaPreemptionDelay - oldDelay)
				log.Log(log.SchedQueue).Info("consecutive change of quota with delay change",
					zap.String("queue", sq.QueuePath),
					zap.Time("quotaPreemptionStartTime", sq.quotaPreemptionStartTime),
					zap.Duration("delta", sq.quotaPreemptionDelay-oldDelay))
				return
			}
			log.Log(log.SchedQueue).Info("consecutive change of quota: preemption time already set from earlier quota change",
				zap.String("queue", sq.QueuePath),
				zap.Time("quotaPreemptionStartTime", sq.quotaPreemptionStartTime))
		}
	}
...
func (sq *Queue) UpdateQueueProperties(oldMaxResource *resources.Resource) {
	...
	for key, value := range sq.properties {
		...
		case configs.PreemptionDelay:
			if sq.isLeaf {
				sq.preemptionDelay, err = convertDelay(value, configs.DefaultPreemptionDelay)
				if err != nil {
		...
		case configs.ApplicationUnschedulableAsksBackoff:
			sq.unschedAskBackoff, err = unschedulableAskBackoff(value)
			if err != nil {
		...
		case configs.ApplicationUnschedulableAsksBackoffDelay:
			sq.askBackoffDelay, err = convertDelay(value, configs.DefaultAskBackOffDelay)
			if err != nil {
...
// The state machine has its own locking, the queue lock guards the state time update.
// +checklocks:sq.RWMutex
...
func (sq *Queue) MarkQueueForRemoval() {
	...
	log.Log(log.SchedQueue).Info("marking managed queue for deletion",
		zap.String("queue", sq.QueuePath))
	sq.doRemoveQueue()
	// YUNIKORN-3419: guarded queue field read without the lock (parent-first path / after release / loop variable)
	if len(sq.children) > 0 { // +checklocksignore
		for _, child := range children {
...
func (sq *Queue) TryIncAllocatedResource(alloc *resources.Resource) error {
	...
	if !sq.allocatedResFits(alloc) {
		return fmt.Errorf("allocation (%v) puts queue '%s' over maximum allocation (%v), current usage (%v)",
			alloc, sq.QueuePath, sq.maxResource, sq.allocatedResource) // +checklocksignore
	}
		...
		if err := sq.parent.TryIncAllocatedResource(alloc); err != nil {
			...
			if sq.isLeaf { // +checklocksignore
				log.Log(log.SchedQueue).Warn("parent queue exceeds maximum resource",
					...
					zap.Stringer("queueUsage", sq.allocatedResource), // +checklocksignore
					zap.Stringer("maxResource", sq.maxResource),      // +checklocksignore
					zap.Error(err))
			}
			return err
		}
...
func (sq *Queue) DecAllocatedResource(alloc *resources.Resource) error {
	...
	if alloc != nil && !sq.resourceFitsAllocated(alloc) {
		return fmt.Errorf("released allocation (%v) is larger than '%s' queue allocation (%v)",
			alloc, sq.QueuePath, sq.allocatedResource) // +checklocksignore
	}
	// check the parent: need to pass before updating
	if sq.parent != nil {
...
func (sq *Queue) getOutStandingRequestsInternal(parentHeadroom *resources.Resource, total *[]*Allocation) *resources.Resource {
	...
	if sq.IsLeafQueue() {
		...
		for _, app := range sq.sortApplications(false) {
			...
			// YUNIKORN-3419: application fields read without the application lock while walking the sorted application list
...
func (sq *Queue) findPreemptionFenceRoot(priorityMap map[string]int64, currentPriority int64, askResource *resources.Resource) *Queue {
	...
	if maxResource != nil && len(maxResource.Resources) > 0 {
		// YUNIKORN-3419: same as above
		projected := resources.Add(sq.allocatedResource, askResource) // +checklocksignore
