...
func TestPlaceholderTimeoutReached(t *testing.T) {
	siApp := &si.AddApplicationRequest{}
	user := security.UserGroup{
		User:   "testuser",
		Groups: []string{},
	...
	app := NewApplication(siApp, user, nil, "")
	assert.Equal(t, app.execTimeout, time.Millisecond)

	// 20min timeout --> no timeout, corrected to 10min
	siApp.ExecutionTimeoutMilliSeconds = (20 * time.Minute).Milliseconds()
	app = NewApplication(siApp, user, nil, "")
	remainingTime := app.execTimeout.Round(time.Minute)
	assert.Equal(t, remainingTime, 10*time.Minute)

	// 20min timeout, no creationTime --> no change
	siApp.Tags = map[string]string{}
	siApp.ExecutionTimeoutMilliSeconds = (20 * time.Minute).Milliseconds()
	app = NewApplication(siApp, user, nil, "")
	assert.Equal(t, app.execTimeout, 20*time.Minute)
