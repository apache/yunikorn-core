...
func TestTryAllocateMaxApps(t *testing.T) {
	partition := createQueuesNodes(t)
	partition.getQueueInternal("root.leaf").SetMaxRunningApps(uint64(1))
	if partition == nil {
		t.Fatal("partition create failed")
	}
	if alloc := partition.tryAllocate(); alloc != nil {
	...
	// create a set of queues and apps: app-1 2 asks; app-2 2 ask (same size)
	// app-2 will not become running status, so that app-2 should be allocated only once
	...
	err = app.AddAllocationAsk(newAllocationAsk("alloc-1", appID1, res))
	assert.NilError(t, err, "failed to add ask alloc-1 to app-1")
	err = app.AddAllocationAsk(newAllocationAsk("alloc-2", appID1, res))
	assert.NilError(t, err, "failed to add ask alloc-2 to app-1")
	...
	err = partition.AddApplication(app)
	assert.NilError(t, err, "failed to add app-2 to partition")
	err = app.AddAllocationAsk(newAllocationAsk("alloc-1", appID2, res))
	assert.NilError(t, err, "failed to add ask alloc-1 to app-2")
	err = app.AddAllocationAsk(newAllocationAsk("alloc-1", appID2, res))
	assert.NilError(t, err, "failed to add ask alloc-2 to app-2")
	...
	assert.Equal(t, partition.GetQueue("root.leaf").GetRunningApps(), uint64(1))
	...
	// third allocation should be app-2 and alloc-1
	alloc = partition.tryAllocate()
	if alloc != nil {
		t.Fatal("maxApps did not work")
	}
	assert.Equal(t, partition.GetQueue("root").GetRunningApps(), uint64(1))
	assert.Equal(t, partition.GetQueue("root.leaf").GetRunningApps(), uint64(1))

	partition.removeApplication(appID1)
	assert.Equal(t, partition.GetQueue("root").GetRunningApps(), uint64(0))
	assert.Equal(t, partition.GetQueue("root.leaf").GetRunningApps(), uint64(0))
